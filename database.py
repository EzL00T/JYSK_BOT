...
# Сетка бонусов согласно шкале JYSK
BONUS_TABLE = {
    100: 0.0,
    105: 15.0,
    110: 20.0,
...
def get_bonus_percent(efficiency: float) -> float:
    """Определяет процент надбавки к зарплате по нижней ступени эффективности."""
...
async def add_boxes(user_id: int, boxes: int) -> dict:
    today = get_today_date()
    shift = await get_shift(user_id, today)
    new_boxes = shift["boxes"] + boxes
    return await set_boxes(user_id, new_boxes, today)
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("""
            ...
            ON CONFLICT(user_id, shift_date) DO UPDATE SET
                ...
                status = CASE WHEN status IN ('VACATION', 'SICK', 'OFF') THEN 'WORK' ELSE status END
...
async def get_monthly_leaderboard(month_prefix: str = None) -> list:
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute("""
            ...
            FROM shifts s
            JOIN users u ON s.user_id = u.user_id
            WHERE s.shift_date LIKE ? AND s.status IN ('WORK', 'OVERTIME_DAY')
